      ...
      {demos.length === 0 ? (
        ...
        <section className="mt-6 grid gap-5">{demos.map((demo) => { const summary = summarizeClientReadiness(demo.checks); return (
          <article className="rounded-xl border border-[var(--line)] bg-white p-5" key={demo.organizationId}>
            ...
            <div className="mt-5 flex flex-wrap gap-2"><form action={prepareReferenceDemoAction}><input name="organization_id" type="hidden" value={demo.organizationId} /><input name="venue_id" type="hidden" value={demo.venueId} /><button className="min-h-11 rounded-lg bg-[var(--accent)] px-4 text-sm font-black text-white" type="submit">Prepare reference demo</button></form><Link className="inline-flex min-h-11 items-center rounded-lg border border-[var(--line)] px-4 text-sm font-black" href="/admin/command-center">Command Center</Link><Link className="inline-flex min-h-11 items-center rounded-lg border border-[var(--line)] px-4 text-sm font-black" href={`/admin/pilot-launch?venueId=${demo.venueId}`}>Pilot Launch</Link><a className="inline-flex min-h-11 items-center rounded-lg border border-[var(--line)] px-4 text-sm font-black" href={getPublicVenueUrl(demo.venueId)} target="_blank" rel="noreferrer">Public venue</a></div>
          </article>
        ); })}</section>
      )}
