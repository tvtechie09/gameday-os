...
export type ProviderRouteContext = { params: Promise<{ provider: string }> };
...
export function integrationApiError(error: unknown) {
  ...
  console.error("Integration provider API failure", error);
  return NextResponse.json({ error: error instanceof Error ? error.message : "Integration provider request failed safely.", ok: false }, { status: 500 });
}
