...
import { AutomationError, disableAutomationWorkflow } from "@/lib/services/automation-workflows";
...
function automationApiError(error: unknown) {
  ...
  console.error("Automation disable API failure", error);
  return NextResponse.json({ error: "Automation disable failed safely.", ok: false }, { status: 500 });
}

export async function POST(request: Request, context: RouteContext) {
  try {
    const { id } = await context.params;
    const workflow = await disableAutomationWorkflow(id, await getVerifiedVenueActorId());
