...
test("explicit Pilot environment hides and disables development login tooling", () => {
  const build = readFileSync("src/lib/pilot-build.ts", "utf8");
  const env = readFileSync("src/lib/access/env.ts", "utf8");
  const login = readFileSync("src/app/login/page.tsx", "utf8");
  assert.match(build, /PILOT_PREVIEW/);
  assert.doesNotMatch(build, /VERCEL_GIT_COMMIT_REF|security\/audit-remediation-2026-08-28/);
