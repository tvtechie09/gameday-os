...
function devActor(key: keyof typeof identities): AccessContext {
  ...
  const actor = actorFromDevSession(payload);
  assert.ok(actor);
  return actor;
}

function hostedActor(key: keyof typeof identities): AccessContext {
  const identity = identities[key];
  const snapshot: CanonicalActorSnapshot = {
    authUserId: `auth-${key}`,
    userId: `user-${key}`,
    email: `${key}@nlsa.test`,
    displayName: key,
    userStatus: "active",
    assignments: [{ roleId: `role-${key}`, ...identity, permissionKeys: [...identity.permissions], assignmentStatus: "approved" }],
    existingVenueIds: new Set(),
  };
...
test("NLSA routes are private and map to one explicit experience", () => {
  ...
  assert.match(proxy, /pathname === "\/demo\/nlsa"/);
  assert.match(proxy, /canAccessNlsaExperience/);
...
test("NLSA role matrix allows only the intended positive paths", () => {
  for (const resolve of [devActor, hostedActor]) {
    const owner = resolve("owner");
    const staff = resolve("staff");
    const coach = resolve("coach");
    const family = resolve("family");
...
test("NLSA identities land inside their least-privilege experience", () => {
  assert.equal(getRoleHome(devActor("owner")), "/demo/nlsa/today");
  assert.equal(getRoleHome(devActor("staff")), "/demo/nlsa/operations");
  assert.equal(getRoleHome(devActor("coach")), "/demo/nlsa/coach");
  assert.equal(getRoleHome(devActor("family")), "/demo/nlsa/family");
  const proxy = readFileSync(new URL("../src/proxy.ts", import.meta.url), "utf8");
  assert.match(proxy, /const nlsaHome = getNlsaHome\(ctx\)/);
  assert.match(proxy, /home\.searchParams\.set\("denied", pathname\)/);
});
